...
class login_completion_handler : public net_socket::recv_completion_handler
{
public:
  std::shared_ptr<boost_socket> my_sock_;
  login_completion_handler(std::shared_ptr<boost_socket> my_sock)
    : my_sock_(my_sock)
  {
  ...
  virtual void on_recv(const net_socket::buffer& data, unsigned size) override
  {
    ...
    login_protocol::to_client_packet_opcodes opcode;
    bd >> opcode;
    switch (opcode)
    {
      case login_protocol::to_client_packet_opcodes::account_already_online:
        puts("account_already_online");
        break;
      case login_protocol::to_client_packet_opcodes::here_have_a_cookie:
      {
        std::string cookie;
        bd >> cookie;
      ...
      case login_protocol::to_client_packet_opcodes::wrong_username:
        puts("wrong_username");
        break;
      case login_protocol::to_client_packet_opcodes::wrong_password:
        puts("wrong_password");
        break;
      default:
        printf("HUUH unknown opcode for lgin %d\n", opcode);
        break;
    }
  }
  virtual void on_error() override { puts("error, dupa"); }
};

class my_recv_completion_handler : public net_socket::recv_completion_handler
{
public:
  std::shared_ptr<boost_socket> my_sock_;
          ...
          case ingame_state_protocol::to_client_packet_opcodes::friend_add_result:
          {
            ...
            switch (result)
            {
              case friend_add_request_result::previous_still_waiting:
                printf(
                  "CHILL THE FUCK OUT, YOUR PREVIOUS REQUEST to %s IS STILL WAITIN FOR RESPONSE\n",
                  username.c_str());
                break;
...
void
handle_ingame_command(char command, std::shared_ptr<boost_socket> socket)
{
...
void
handle_lobby_command(char command, std::shared_ptr<boost_socket> socket)
{
  net_socket::buffer sendbuf;
  switch (command)
  {
    ...
    {
      binary_serializer bs(sendbuf);
      bs << lobby_state_protocol::to_server_packet_opcodes::log_off
         << 321564; // btw redundant data should be OK
      socket->send_to_client(sendbuf, bs.get_current_size());
    }
    break;
    default:
...
main()
{
  ...
  boost::asio::strand<boost::asio::io_context::executor_type> sync_strand =
    boost::asio::make_strand(io_service);
  ...
  // temp
  auto ep = boost::asio::ip::tcp::endpoint(boost::asio::ip::address::from_string(ip, ec), 6789);
  if (ec)
  {
  ...
  std::thread([&socket] {
    net_socket::buffer sendbuf;
    std::cout << "Enter username and pass :\n";
    std::string login, pass;
    std::cin >> login >> pass;
    binary_serializer bs(sendbuf);
    // std::string loginkeys(64, 'x');
    bs << login_protocol::to_server_packet_opcodes::request_login << login << pass;
    // bs << preauth_state_protocol::to_server_packet_opcodes::loginserver_keys << loginkeys <<
    // login;
    socket->send_to_client(sendbuf, bs.get_current_size());
    return;
    while (1)
  ...
  login_completion_handler tralala(socket);
  io_service.run();
}
