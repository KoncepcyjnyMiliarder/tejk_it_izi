...
public:
  asynchronous_database_adapter(database_facade& synchronous_db,
                                task_scheduler& game_loop_scheduler,
  ...
  void get_lobby_chars(unsigned account_id, CharacterListCallback&& cb)
  {
    database_task_scheduler_.schedule_task([this, account_id, cb] {
      game_loop_scheduler_.schedule_task(
        std::bind(cb, synchronous_db_.get_lobby_chars(account_id)));
    });
  }
  /*
  ...
  virtual lobby_character create_character(const std::string& nickname, unsigned
  owner_acc_id) = 0; virtual account_data get_account_data(const std::string&
  login) = 0; virtual std::unordered_set<unsigned> get_friends_of(unsigned
  char_id) = 0; virtual void make_friends(unsigned one, unsigned other) = 0;
