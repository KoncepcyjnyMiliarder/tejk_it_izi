...
class tii_entity_representative
{
  ...
  public:
    ...
    struct less
    {
      bool operator()(const tii_entity_representative& lhs, const tii_entity_representative& rhs) const
      {
        return lhs.uid() < rhs.uid();
      }
    };

    tii_entity_representative(const lobby_character& selected_char, std::shared_ptr<net_session> my_session);
