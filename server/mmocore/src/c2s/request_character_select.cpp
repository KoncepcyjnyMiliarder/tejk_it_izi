...
request_character_select::request_character_select(
...
void
request_character_select::execute_associated_action()
{
  auto it =
    std::find_if(lobby_chars_.begin(),
                 lobby_chars_.end(),
                 [this](const lobby_character& character) { return character.name == char_name_; });

  if (it == lobby_chars_.end())
  {
    logger_.log_diagnostic(acc_data_.login +
                           " requested to join game with nonexistent character: " + char_name_);
  ...
  transitioner_.schedule_transition(std::make_unique<ingame_state>(
    transitioner_, my_session_, world_, logger_, *it, db_, acc_data_, async_db_));
}
