...
void pending_friend_requests::periodic_cleanup()
{
  //THIS IS SO BEAUTIFUL
  pending_friend_requests_back_.clear();
  std::swap(pending_friend_requests_front_, pending_friend_requests_back_);
  periodic_cleanup_task_ = std::move(scheduler_.postpone_task(std::bind(&pending_friend_requests::periodic_cleanup, this), boost::posix_time::seconds(20)));
}
...
pending_friend_requests::operation_result pending_friend_requests::process_add_friend_request(unsigned sender, unsigned recipient, std::chrono::system_clock::time_point current_time)
{
  ...
  {
    auto seconds = std::chrono::duration_cast<std::chrono::seconds>(current_time - back_iter->second);
    if (seconds.count() < 20)
    {
