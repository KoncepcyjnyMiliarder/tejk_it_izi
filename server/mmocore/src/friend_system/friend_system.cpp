...
friend_system::friend_system(world_registrar& online_player_registry,
...
void
friend_system::register_me(tii_entity_representative::const_reference my_representative,
                           std::unique_ptr<friend_backend> my_backend)
{
  assert(friendlist_of_id_.count(my_representative.uid()) == 0);
  friendlist& my_friendlist =
    friendlist_of_id_
      .emplace(my_representative.uid(),
...
void
friend_system::unregister_me(tii_entity_representative::const_reference my_representative)
{
...
void
friend_system::process_req_friend_add(tii_entity_representative::const_reference my_representative,
                                      const std::string& name)
{
  ...
  // if he has been found online, it means he also has to be registered in the
  // friendsystem - invariant!
  ...
  auto request_state = pending_requests_.process_add_friend_request(
    my_representative.uid(), to_add_id, clock_.get_current_time());
  switch (request_state)
  {
    case pending_friend_requests::operation_result::already_in_buffer:
      // well well welly my friend... the old request didnt expire yet, chill the
      // fuck out!
...
void
friend_system::process_req_friend_remove(
  tii_entity_representative::const_reference my_representative,
  const std::string& name)
{
  assert(friendlist_of_id_.count(my_representative.uid()));
