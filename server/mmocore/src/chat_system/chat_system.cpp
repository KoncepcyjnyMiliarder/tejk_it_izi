...
void
chat_system::unregister_from_chatroom(tii_entity_representative::const_pointer my_representative,
                                      chatroom* room)
{
  // this only erases me from chatroom map, i got to erase the chatroom from my
  // own set on my own
  assert(room);
  ...
  for (auto& participant : room->participants)
    entities_.at(participant)
      .backend->on_user_left_chat(room->room_name, my_representative->name());
}

void
chat_system::periodic_broadcast()
{
  for (auto& chatroom_pair : active_chatrooms_)
  {
    ...
    const std::string message(
      "There are currently: " + std::to_string(selected_channel.participants.size()) +
      " users on the chat!");
    for (auto& participant : selected_channel.participants)
      entities_.at(participant)
        .backend->on_chat_message(selected_channel.room_name, sender_name, message);
  }
  periodic_broadcast_task_ = std::move(scheduler_.postpone_task(
    std::bind(&chat_system::periodic_broadcast, this), boost::posix_time::seconds(6)));
}

chat_system::chat_system(task_scheduler& scheduler, world_registrar& online_player_registry)
  : scheduler_(scheduler)
  , online_player_registry_(online_player_registry)
{
  periodic_broadcast_task_ = std::move(scheduler_.postpone_task(
    std::bind(&chat_system::periodic_broadcast, this), boost::posix_time::seconds(6)));
}
...
void
chat_system::register_me(tii_entity_representative::const_reference my_representative,
...
void
chat_system::unregister_me(tii_entity_representative::const_reference my_representative)
{
...
void
chat_system::join_chatroom(tii_entity_representative::const_reference my_representative,
...
void
chat_system::leave_chatroom(tii_entity_representative::const_reference my_representative,
                            const std::string& room_name)
{
  assert(entities_.count(&my_representative));
...
void
chat_system::broadcast_message(tii_entity_representative::const_reference my_representative,
                               const std::string& room_name,
                               const std::string& msg)
{
  ...
  for (auto& participant : room->participants)
    entities_.at(participant)
      .backend->on_chat_message(room->room_name, my_representative.name(), msg);
}

void
chat_system::whisper(tii_entity_representative::const_reference my_representative,
