...
void
mmoclient::on_recv(const net_socket::buffer& data, unsigned size)
{
...
mmoclient::mmoclient(std::shared_ptr<net_socket> sock,
                     database_facade& db,
...
void
mmoclient::start(world& universe, login_validator& authenticator)
{
  logger_.log_diagnostic(__func__);
  current_state_ = std::make_unique<preauth_state>(
    *this, shared_from_this(), universe, logger_, authenticator, db_, async_db_);
  current_state_->start();
}

void
mmoclient::force_close()
{
  logger_.log_diagnostic(__func__);
...
void
mmoclient::schedule_transition(std::unique_ptr<client_state> new_state)
{
  logger_.log_diagnostic(__func__);
  ...
  scheduler_.schedule_task([this] {
    // delete the old state, and if it didnt trigger any database tasks, just
    // start the new one
