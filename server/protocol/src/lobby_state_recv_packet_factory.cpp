...
received_packet::packet_ptr lobby_state_recv_packet_factory::construct(const std::array<char, 2048>& data, unsigned size)
{
  ...
  switch (opcode)
  {
    ...
    case lobby_state_protocol::delete_character:
      return std::make_unique<request_character_delete>(bd, my_session_, logger_, lobby_chars_, db_);
