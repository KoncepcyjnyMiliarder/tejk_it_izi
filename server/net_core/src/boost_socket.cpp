...
boost_socket::do_recv_expected_len(recv_completion_handler& handler)
{
  async_read(
    ...
    boost::asio::bind_executor(sync_strand_,
                               [this, &handler, self = shared_from_this()](
...
{
	boost::asio::post(sync_strand_, std::bind(&boost_socket::do_recv_expected_len, this, std::ref(handler)));
