...
tejk_it_izi::tejk_it_izi(database_facade& db,
...
tejk_it_izi::tick()
{
  ...
  timer_.async_wait(boost::asio::bind_executor(sync_strand_, std::bind(&tejk_it_izi::tick, this)));
}
...
void
tejk_it_izi::on_accept(std::shared_ptr<net_socket> sock)
{
  logger_.log_diagnostic(__func__);
  std::make_shared<mmoclient>(std::move(sock), db_, scheduler_, logger_, async_db_)
    ->start(universe_, authenticator_);
}
