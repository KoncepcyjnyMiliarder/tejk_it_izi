...
fake_database_facade::get_lobby_chars(unsigned account_uid)
{
  return account_to_chars_[account_uid];
}

void
fake_database_facade::erase_character(const lobby_character& character)
{
  for (auto& map_node : account_to_chars_)
  {
    auto& char_list = map_node.second;
    char_list.remove_if(
      [&character](const lobby_character& elem) { return elem.name == character.name; });
  }
}

bool
fake_database_facade::is_nickname_already_taken(const std::string& nickname)
{
  return char_name_to_uid(nickname) != 0;
}

unsigned
fake_database_facade::char_name_to_uid(const std::string& nickname)
{
  for (auto& map_node : account_to_chars_)
  {
    auto& char_list = map_node.second;
    auto iter =
      std::find_if(char_list.begin(), char_list.end(), [&nickname](const lobby_character& elem) {
...
lobby_character
fake_database_facade::create_character(const std::string& nickname, unsigned owner_acc_id)
{
...
account_data
fake_database_facade::get_account_data(const std::string& login)
{
  return acc_datas_.at(login);
}

std::unordered_set<unsigned>
fake_database_facade::get_friends_of(unsigned char_id)
{
...
void
fake_database_facade::make_friends(unsigned one, unsigned other)
{
...
void
fake_database_facade::remove_friends(unsigned one, unsigned other)
{
